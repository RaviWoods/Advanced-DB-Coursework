...
FROM (
	...
	FROM(	SELECT ALL zip_code, zip_name, name, (3959 * acos((sin(radians(zip.latitude))*sin(radians(place.latitude))) + (cos(radians(zip.latitude))*cos(radians(place.latitude))*cos(radians(place.longitude-zip.longitude))))) as distance
